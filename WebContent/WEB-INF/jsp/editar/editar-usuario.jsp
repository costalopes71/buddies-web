...
<body>
							...
							<div class="row">
								...
								<div class="form-group col-md-2">
									...
									<form:select path="administrador" cssClass="custom-select form-control" id="administrador">
										<form:option label="${usuario.administrador ? 'Sim' : 'Não'}" value="${usuario.administrador}"/>
										<form:option label="${usuario.administrador ? 'Não' : 'Sim'}" value="${usuario.administrador ? 'false' : 'true'}"/>	
									</form:select>
									<span class="text-danger" id="erroAdministrador"></span>
								</div>
							</div>
	...
	<!-- INICIO DOS SCRIPTS PROPRIOS DA PAGINA DE EDITAR USUARIO -->
	<script src="<c:url value="/resources/js/validacao/validacao_usuarios.js" />"></script>
	<!-- FIM DOS SCRIPTS PROPRIOS DA PAGINA DE EDITAR USUARIO -->
